...
StreamReassembler::StreamReassembler(const size_t capacity)
    ...
    , _output(capacity)
    , _capacity(capacity) {}
//! \details This functions calls just after pushing a substring into the _output stream.
//! It aims to check if there exists any contiguous substrings recorded earlier can be push ito the stream.
...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if (index >= unass_base) {
        ...
        size_t offset = unass_base - index;
